...
declare_id!("GecnYmwfw5naFyJZnzZrqVvXXmEnmw7tK5p17NCGv57u");
...
pub mod anchor_nft_staking {
    ...
    pub fn stake(ctx: Context<Stake>) -> Result<()> {
        ...
        ctx.accounts.general_state.tokens_staked += 1;
        msg!("Nfts Staked: {:?}");
...
pub struct Stake<'info> {
    ...
    #[account(
        init_if_needed,
        payer=user,
        space = std::mem::size_of::<GeneralInfoStake>() + 8,
        seeds = [user.key().as_ref(), stake_mint.key().as_ref()],
        bump
    )]
    pub general_state: Account<'info, GeneralInfoStake>,
...
pub struct Redeem<'info> {
    ...
    pub stake_state: Account<'info, UserStakeInfo>,
    #[account(mut)]
    pub stake_mint: Account<'info, Mint>,    #[account(
        mut,
        seeds = [user.key().as_ref(), stake_mint.key().as_ref()],
        bump,
...
#[account]
pub struct GeneralInfoStake {
    pub tokens_staked: i16,
    pub tokens_owed: i16,
    pub last_staked: i16,
}
